...
public class GradeBook01 {
	...
	public static void print() {
		if (ArrayUtil.isEmpty(idArray)) {
			...
			//사용자로부터 몇 번 인덱스의 학생을 볼 지 입력을 받는다.
			//단, 한 가지 주의할 것은 사용자가 입력할 번호와
			//실제 인덱스가 다르다는 것이다. 
			String message = "상세보기 할 학생의 번호를 입력하세요.\n뒤로 가시려면 0을 눌러주세요.";
			int userChoice = ScannerUtil.nextInt(scanner, message, 0, idArray.length )-1;
			if(userChoice != -1) {
				printByIndex(userChoice); //뒤로 가기 기능
	...
	public static void delete(int index) {
		//사용자로부터 정말로 삭제할 지 물어봐서
		//1이라고 입력했을 때에만 삭제하고 메뉴보기로 돌아간다. 
		//그 외의 값은 무조건 동의하지 않은 걸로 보고
		//다시 개별 보기로 돌아간다. 
		
		String message = "정말로 삭제하시겠습니까? (1:예 2: 아니요)";
		int userChoice = ScannerUtil.nextInt(scanner, message);
		if(userChoice ==1) {
			//각 배열의 index 번 캄을 삭제한 후에 
			//목록 보기로 돌아간다. 
