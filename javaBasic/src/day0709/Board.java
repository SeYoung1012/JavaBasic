...
//인덱스. 제목이 출력되며
//개별 글 보기 기능이 있어야 합니다. 
//또한 개별 글 보기 시에는 수정/삭제 기능이 있어야 합니다.
//4시 20분까지 
...
// 새 글쓰기를 담당하는 write()
	...
	public static void printAll() {
		if (!ArrayUtil.isEmpty(idArray)) {
			...
			// 코드 실행 순서에 따라 자동으로 showMenu()의 while (true)로 돌아가게 된다.

			// 입력 시에 사용할 메세지를 저장할 변수

			String message = "상세보기 할 글의 번호나 뒤로 가시려면 0을 입력해주세요.";
	...
	public static void printOne(int index) {
		...
		// 개별 글 출력이 끝나면
		// 사용자게에 수정, 삭제,목록으로 가기 중 하나를 입력하도록 안내한다.
	...
	public static void update(int index) {
		// 글을 수정할 때에는 번호와 작성자는 수정되어선 안되므로
		// 제목과 내용을 새로 입력을 받아서 저장한다.
		...
		// 수정 후 해당 index의 개별 글 보기를 실행해서 수정 결과를 볼 수 있게 메소드를 호출한다.
		printOne(index);

	}

	public static void delete(int index) {
		// 사용자로부터 정말로 삭제할 것인지를 입력받아서
		// 삭제를 원하면 각ㄱ 배열에서 해당 인덱스를 삭제하고
		// 목록 보기로 이동
